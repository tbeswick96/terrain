...
public class VolumeClouds_SM : MonoBehaviour {
		...
		Particle[] particlesL;
		ParticleEmitter LegacyParticle;
	...
	public void convertToShuriken(){
			...
			if (this.gameObject.GetComponent<ParticleSystem> () == null) {				
				...
				if (EmitterLegacy == null) {
					passMaxSize = TMPPS.startSize;
					passCount = TMPPS.maxParticles;
				}
			}
			...
			TMPPS.maxParticles = (int)passCount;
			TMPPS.startSize = passMaxSize;
			TMPPS.startLifetime = 100000;
			TMPPS.simulationSpace = ParticleSystemSimulationSpace.World;
			this.useShuriken = true;

			//v3.4.5b - shaping
			ParticleSystem.ShapeModule Shaper = TMPPS.shape;
			Shaper.shapeType = ParticleSystemShapeType.Box;
			Shaper.box = new Vector3 (111, 1, 111);
	...
	void Start () {
			...
			if (this.GetComponent<ParticleRenderer> () != null) {
				...
				LegacyParticle = this.gameObject.GetComponent<ParticleEmitter> ();
			}
			if (Prender != null) {
...
//			}
			...
			int psize = 0;
			if (useShuriken) {
				psize = (int)GetComponent<ParticleSystem> ().maxParticles;

				int SizeP = ((int)(psize / divider)) * (divider);
				GetComponent<ParticleSystem> ().maxParticles = SizeP; //+ (divider)*1;
				GetComponent<ParticleSystem> ().maxParticles = SizeP; //+ (divider)*1;
				//}
	...
	void Update () {
		...
		for (int i=0;i<SunShafts.Count;i++){
			...
			if (ShurikenParticle != null) {
				...
				if(particles == null){
					particles = new ParticleSystem.Particle[ShurikenParticle.maxParticles];
				}
							...
							if(VerticalForm){
								for (int j=0; j < divider/2;j++){
									for (int i=(j*particles_per_cloud); i<((j+1)*particles_per_cloud); i++) {
										...
										particles[i].lifetime = 100000; //v3.4.5a
										particles[i].startLifetime = 100000;
										particles[start_C].lifetime = 100000; //v3.4.5a
					...
					//					particles[i].energy = 2;//Random.Range(1,4);
									...
									if(StableRollMethod2){
										...
										if(!StableRollAllAxis){
											...
											//particles[i].axisOfRotation

											//particles[i].rotation = particles[i].rotation+diffRot+diffRotY*Dot_check;
											//particles[i].axisOfRotation = Vector3.up;
											//particles[i].rotation3D = Vector3.forward;
										}
									}
			...
			if(LegacyParticle!=null){
				...
				//v3.4.6
				if(particlesL == null){
					//Particle[] particles = new Particle[(int)LegacyParticle.maxEmission];//v3.5
					particlesL = new Particle[(int)LegacyParticle.maxEmission];
				}

				if (DecoupledWind) {
					particlesL = LegacyParticle.particles;
				}
		...
		if(Time.fixedTime - Prev_time  > UpdateInterval | !Init){
			...
			//		Particle[] particles = GetComponent<ParticleEmitter>().particles;
					...
					//find particle number for each center
						int particles_per_cloud = (int)(particlesL.Length/divider);
					...
					if(!init_centers){
						int count_me = 0;
							...
							if(VerticalForm){
								for (int j=0; j < divider/2;j++){
									for (int i=(j*particles_per_cloud); i<((j+1)*particles_per_cloud); i++) {
										particlesL[i].position = (particlesL[i].position  - This_transf.position)*Scale_per_cloud[j] + Centers[j] ;
										
										particlesL[i].color = Global_tint + MainColor;
										
										int start_C = (i+(divider/2)*particles_per_cloud);
										//int end_C = ((i+(divider/2))*particles_per_cloud);
										
										particlesL[start_C].position = (particlesL[start_C].position + new Vector3(0,Yspread,0) - This_transf.position)*Scale_per_cloud[j]*YScaleDiff + Centers[j]+MaxCloudDist ;
										
										particlesL[start_C].color = Color.Lerp(Global_tint + MainColor , HeightTint,HeightTint.a);
										
										//v2.2
										if(SmoothIn){
											particlesL[i].color = Color.Lerp(particlesL[i].color,Global_tint + MainColor,Time.deltaTime*SmoothInSpeed);
											particlesL[start_C].color = Color.Lerp(particlesL[i].color,Color.Lerp(Global_tint + MainColor, HeightTint,HeightTint.a),Time.deltaTime*SmoothInSpeed);
											//Debug.Log(HeightTint.a);
										}else{
											particlesL[i].color = Global_tint + MainColor;
											particlesL[start_C].color = Color.Lerp(Global_tint + MainColor, HeightTint,HeightTint.a);
						...
						if(	count_me > 0){
							init_centers=true; Init = true; 
								LegacyParticle.particles = particlesL;
						}
					}else{
						...
						for (int j=0; j < divider;j++){
							...
							for (int i=(j*particles_per_cloud); i<((j+1)*particles_per_cloud); i++) {
								...
								int Closest_Center_ID=j;
								for (int k=0; k<Centers.Count; k++) {
										if(Vector3.Distance(Centers[k],particlesL[i].position) < Vector3.Distance(Closest_center,particlesL[i].position)){
										Closest_center = Centers[k];
										Closest_Center_ID=k;
									}
								}
								Dist_sun_center = (Closest_center - keep_sun_transf.position).magnitude;
									...
									particlesL[i].position = particlesL[i].position +wind*(multiplier*speed*(Closest_Center_ID+1))*Time.deltaTime; 
								
									float Dist_sun_part = (particlesL[i].position - keep_sun_transf.position).magnitude;
								float Diff = (Dist_sun_part - (Dist_sun_center))  / 1.380f; // move to each center's center
								
								if(Prev_cam_rot == Cam_transf.eulerAngles){
										particlesL[i].color = Color.Lerp (SunColor,MainColor,Diff);
...
//						}
						...
						if(Time.fixedTime - Cloud_update_current < Cloud_spread_delay){
							...
							for (int i=alternate-2; i<particlesL.Length; i=i+frames_divider) {
							...
							for (int k=0; k<Centers.Count; k++) {
								//
								if(Use2DCheck){
											if(Vector2.Distance(new Vector2(Centers[k].x,Centers[k].z),new Vector2(particlesL[i].position.x,particlesL[i].position.z)) 
												< Vector3.Distance(new Vector2(Closest_center.x,Closest_center.z),new Vector2(particlesL[i].position.x,particlesL[i].position.z))){
							...
							//v.1.7 - sheet handle
							//Random.seed = i;
									particlesL[i].energy = 2;//Random.Range(1,4);
							
							if(Flatten_below){
										float Dist_part_center = (Closest_center.y - particlesL[i].position.y);
								if(Dist_part_center > 0){
											particlesL[i].position = new Vector3(particlesL[i].position.x, particlesL[i].position.y +(500)*Time.deltaTime,particlesL[i].position.z); 
								}
							}
							
									float dist = (keep_sun_transf.position - particlesL[i].position).magnitude/10;
							...
							if(Turbulent){
								...
								if(method ==1){//use for static camera, no extra sorting required, will break if camera rotates
									...
									//particles[i].size = 3f*250*Mathf.Cos (Time.fixedTime+i*Time.fixedTime*0.0005f);
											particlesL[i].size = Mathf.Abs(3f*250*Mathf.Cos (Time.fixedTime+i*Time.fixedTime*0.0005f));
											if(particlesL[i].size < 330){
												particlesL[i].size =330;
											}else if(particlesL[i].size > 600){
												particlesL[i].size =600;
									}
								}
								else if(method ==2){
									
											if(particlesL[i].size < 300 | 1==1){
										
												particlesL[i].size = particlesL[i].size + 4f*200*Mathf.Cos (Time.fixedTime+dist*Time.fixedTime*0.0005f)*Time.deltaTime;
										//particles[i].size = particles[i].size + 4f*200*Mathf.Cos (Time.fixedTime+i*Time.fixedTime*0.0005f)*Time.deltaTime;
									}else{
										//	particles[i].size = particles[i].size - 3f*250*Mathf.Cos (Time.fixedTime+i*Time.fixedTime*0.0005f);
									}
											if(particlesL[i].size < 300){
												particlesL[i].size =300;
											}else if(particlesL[i].size > 500){
												particlesL[i].size =500;
									}
								}
								else if(method == 3){
									...
									COS_IN = Time.fixedTime+dist*0.115f;
											...
											if(particlesL[i].size < 500 & particlesL[i].size > 200){
										
												particlesL[i].size = particlesL[i].size + 4f*200*Mathf.Cos (COS_IN)*Time.deltaTime;
										//particles[i].size = particles[i].size + 4f*200*Mathf.Cos (Time.fixedTime+i*Time.fixedTime*0.0005f)*Time.deltaTime;
									}else{
										//	particles[i].size = particles[i].size - 3f*250*Mathf.Cos (Time.fixedTime+i*Time.fixedTime*0.0005f);
									}
											if(particlesL[i].size < 200){
										//particles[i].size =100;//keep growing, but slower, until cos turns it around
										if(Mathf.Cos (COS_IN) <0){
													particlesL[i].size = particlesL[i].size + 0.4f*200*Mathf.Cos (COS_IN)*Time.deltaTime;
										}else{
													particlesL[i].size = particlesL[i].size + 4.4f*200*Mathf.Cos (COS_IN)*Time.deltaTime;
										}
											}else if(particlesL[i].size > 500){
										//particles[i].size =500;
										//keep growing, but slower, until cos turns it around
										...
										particlesL[i].position = particlesL[i].position +wind*(multiplier*speed*(dist+1))*Time.deltaTime;
							}else{
									if(!DecoupledWind){
											particlesL[i].position = particlesL[i].position +wind*(multiplier*speed*(Closest_Center_ID+1))*Time.deltaTime; 
									}
							...
							if(EnableLOD & Prev_cam_rot == Cam_transf.eulerAngles){
								//float LodMaxYDiff = 140;
								//float LodMaxHDiff = 700;
										float DistCP = Vector2.Distance(new Vector2(Cam_transf.position.x,Cam_transf.position.z),new Vector2(particlesL[i].position.x,particlesL[i].position.z));
								//if(Mathf.Abs(Cam_transf.position.y - particles[i].position.y) < LodMaxYDiff 
								if(//Mathf.Abs(Cam_transf.position.y - Closest_center.y) < LodMaxYDiff &
									//Vector2.Distance(new Vector2(Cam_transf.position.x,Cam_transf.position.z),new Vector2(particles[i].position.x,particles[i].position.z)) > (LodMaxHDiff * (0.7f+(Dist_hero_center/LodMaxYDiff)))
									 (DistCP > LodMaxHDiff | DistCP < LODMinDist)
									...
									if(ParticlesLODed[i] == 0){
										//particles[i].position = new Vector3(particles[i].position.x, particles[i].position.y+LOD_send_height, particles[i].position.z);
										//ParticlesLODed[i] = 1; 
												ParticlesLODed[i] = (int)particlesL[i].size;
										//particles[i].size = 0;
										//particles[i].color = Color.Lerp(particles[i].color,new Color(particles[i].color.r,particles[i].color.g,particles[i].color.b,0),Time.deltaTime*SmoothoutSpeed);
									}
									if(ParticlesLODed[i] > 0){
												particlesL[i].size = Mathf.Lerp(particlesL[i].size, 0 ,Time.deltaTime*LODFadeOutSpeed);
												particlesL[i].color = Color.Lerp(particlesL[i].color,new Color(particlesL[i].color.r,particlesL[i].color.g,particlesL[i].color.b,0),Time.deltaTime*LODFadeOutSpeed*2);//become transp. faster
									}
								}else{
									//if(ParticlesLODed[i] == 1){
									if(ParticlesLODed[i] > 0){
										//particles[i].position = new Vector3(particles[i].position.x, particles[i].position.y-LOD_send_height, particles[i].position.z);
												...
												if(particlesL[i].size < ParticlesLODed[i]){
													particlesL[i].size = Mathf.Lerp(particlesL[i].size, ParticlesLODed[i] ,Time.deltaTime*LODFadeInSpeed*2);//become normal size before appear
													particlesL[i].color = Color.Lerp(particlesL[i].color,new Color(particlesL[i].color.r,particlesL[i].color.g,particlesL[i].color.b,1),Time.deltaTime*LODFadeInSpeed);
											}else{
												ParticlesLODed[i] = 0; 
											}
							...
							Color SunColor2 = SunColor;
							if(Grab_sun_angle){
										float Angle_factor = Vector3.Angle((particlesL[i].position - keep_sun_transf.position),(This_transf.position - keep_sun_transf.position));
								SunColor2.a = SunColor.a/(Mathf.Abs (Angle_factor)*Sun_angle_influence);
							}
			...
			//					if(!DecoupledColor){
							if(Prev_cam_rot == Cam_transf.eulerAngles){
								if(Init_color < Delay_lerp){
										//v2.2
										if(SmoothIn){
												particlesL[i].color = Color.Lerp(particlesL[i].color,Global_tint + Color.Lerp (SunColor2,MainColor,Diff),Time.deltaTime*SmoothInSpeed);
										}else{
												particlesL[i].color = Global_tint + Color.Lerp (SunColor2,MainColor,Diff);
										}
									Init_color++;
								}else{
										//v1.6 - move tint inside
										if(SmoothIn){//v2.2
											if(!DecoupledColor){
													if(VerticalForm & (particlesL[i].position.y - Closest_center.y) > YScaleDiff){
														particlesL[i].color = Color.Lerp(particlesL[i].color,Color.Lerp (particlesL[i].color,  Color.Lerp(Global_tint + Color.Lerp (SunColor2,MainColor,Diff),HeightTint,HeightTint.a),color_speed*Time.deltaTime),Time.deltaTime*SmoothInSpeed);
												}else{
													if(!DecoupledWind){
															particlesL[i].color = Color.Lerp(particlesL[i].color,Color.Lerp (particlesL[i].color, Global_tint + Color.Lerp (SunColor2,MainColor,Diff),color_speed*Time.deltaTime),Time.deltaTime*SmoothInSpeed);
													}
												}
											}
										}else if(SmoothOut){//v2.2
											if(!DecoupledColor){
												...
												if(ScatterMat != null && ScatterMat.HasProperty("_TintColor")){ //v3.2
													...
													particlesL[i].color = Color.Lerp(particlesL[i].color,new Color(particlesL[i].color.r,particlesL[i].color.g,particlesL[i].color.b,0),Time.deltaTime*SmoothoutSpeed*0.5f);
												//Debug.Log(SmoothoutSpeed);
											}
										}
										else{
												if(VerticalForm & (particlesL[i].position.y - Closest_center.y) > YScaleDiff){
													particlesL[i].color = Color.Lerp (particlesL[i].color, Color.Lerp(Global_tint + Color.Lerp (SunColor2,MainColor,Diff),HeightTint,HeightTint.a),color_speed*Time.deltaTime*0.005f);
											}else{
													particlesL[i].color = Color.Lerp (particlesL[i].color, Global_tint + Color.Lerp (SunColor2,MainColor,Diff),color_speed*Time.deltaTime*0.005f);
											}
										}
								}
							}else{
								...
								if(CloudWaves){
									...
									if(ParticlesLODed[i] == 0 & !SmoothIn){
										//particles[i].color = Color.Lerp(particles[i].color,new Color(particles[i].color.r,particles[i].color.g,particles[i].color.b,Mathf.Abs(Mathf.Cos(WaveFreq*Time.fixedTime+particles[i].position.x))),Time.deltaTime*LODFadeInSpeed);
										//particles[i].color =new Color(particles[i].color.r,particles[i].color.g,particles[i].color.b,Mathf.Abs(Mathf.Cos(WaveFreq*Time.fixedTime+0.1f*Mathf.Sin (Vector3.Distance(particles[i].position,This_transf.position)))));
										particlesL[i].color =new Color(particlesL[i].color.r,particlesL[i].color.g,particlesL[i].color.b,Mathf.Abs(Mathf.Cos(WaveFreq*Time.fixedTime+0.005f* (Vector3.Distance(particlesL[i].position,This_transf.position)))));
									}
								}
		...
		//				Particle[] particles = GetComponent<ParticleEmitter> ().particles;
						...
						if (init_centers) 
						{
							...
							//Debug.Log(particles.Length);
								for (int i=0; i<particlesL.Length; i=i+1) {
								
								if(!DifferentialMotion){
										particlesL [i].position = particlesL [i].position + wind * (multiplier * speed*5) * Time.deltaTime; 
								}else{
										particlesL [i].position = particlesL [i].position + wind * (multiplier * speed*5) * Time.deltaTime* (MaxDiffSpeed*((i+MaxDiffOffset)/particlesL.Length)); 
								}

									//v3.1
									if(renewAboveHeight){
										if(particlesL[i].position.y > renewHeight){
											//if(Random.Range(1,1000) == 3){
											particlesL[i].position = new Vector3(particlesL[i].position.x, This_transf.position.y, particlesL[i].position.z);
											//}
										}
										if (particlesL [i].position.y > (renewHeightPercent / 100)*renewHeight) {//just before go away, fade out //v3.3b
											particlesL[i].color = Color.Lerp(particlesL[i].color,new Color(particlesL[i].color.r,particlesL[i].color.g,particlesL[i].color.b,0),Time.deltaTime*renewFadeOutSpeed*5.5f);//v3.3b
										}else{
										//if (particles [i].position.y < (15 / 100) * renewHeight) {
											particlesL[i].color = Color.Lerp(particlesL[i].color,new Color(particlesL[i].color.r,particlesL[i].color.g,particlesL[i].color.b,(Global_tint + Color.Lerp (SunColor,MainColor,0.8f)).a),Time.deltaTime*renewFadeInSpeed*10.5f);
										}

										if (particlesL [i].position.y < This_transf.position.y+0.1*This_transf.position.y) {
											particlesL[i].color = new Color(particlesL[i].color.r,particlesL[i].color.g,particlesL[i].color.b,0);//v3.3b
										}//else
										//if (particles [i].position.y < This_transf.position.y + 0.3 * This_transf.position.y) {
								...
								Vector3 Closest_center = Centers[0];
										...
										if(DecoupledCounter == i || CenterIDs.Count < particlesL.Length){// || Prev_cam_rot != Cam_transf.eulerAngles){
									for (int k=0; k<Centers.Count; k++) {
										//
										if(Use2DCheck){
													if(Vector2.Distance(new Vector2(Centers[k].x,Centers[k].z),new Vector2(particlesL[i].position.x,particlesL[i].position.z)) 
									...
									Closest_center = Centers[Closest_Center_ID];
										...
										if(CenterIDs.Count < particlesL.Length){
									CenterIDs.Add(Closest_Center_ID);
								}
									...
									if(Prev_cam_rot == Cam_transf.eulerAngles ){
										if(Init_color < Delay_lerp){
											...
											if(SmoothIn){//v2.2
												...
												//if(!DecoupledWind){
													...
													particlesL[i].color = Color.Lerp(particlesL[i].color,new Color(particlesL[i].color.r,particlesL[i].color.g,particlesL[i].color.b,0),Time.deltaTime*SmoothoutSpeed*100);
													//Debug.Log(SmoothoutSpeed);
												//}
											}
											else{
													if(VerticalForm & (particlesL[i].position.y - Closest_center.y) > YScaleDiff){
														particlesL[i].color = Color.Lerp (particlesL[i].color, Color.Lerp(Global_tint + Color.Lerp (SunColor2,MainColor,Diff),HeightTint,HeightTint.a),color_speed*Time.deltaTime);
												}else{
														particlesL[i].color = Color.Lerp (particlesL[i].color, Global_tint + Color.Lerp (SunColor2,MainColor,Diff),color_speed*Time.deltaTime);
												}
											}
										}
									}else{
										...
										if(SmoothIn){//v2.2																			
											//particles[i].color = Color.Lerp(particles[i].color,Color.Lerp (particles[i].color, Global_tint + Color.Lerp (SunColor,MainColor,0.8f),color_speed*Time.deltaTime),Time.deltaTime*SmoothInSpeed);
												...
												//particles[i].color = new Color(particles[i].color.r,particles[i].color.g,particles[i].color.b,Mathf.Lerp(particles[i].color.a,(Global_tint + Color.Lerp (SunColor,MainColor,0.8f)).a,Time.deltaTime*SmoothInSpeed));
											particlesL[i].color = Color.Lerp(particlesL[i].color,Color.Lerp (particlesL[i].color, Global_tint + Color.Lerp (SunColor,MainColor,0.8f),color_speed*Time.deltaTime),Time.deltaTime*SmoothInSpeed*0.5f);
...
//								}
								...
								if(StableRollMethod2){
										...
										if(!StableRollAllAxis){
											...
											if(diffRot1 == 0 && currentRotY1!=Prev_cam_rot.y && Mathf.Abs(Vector3.Angle(Cam_transf.forward, Vector3.up))< 65){//Mathf.Abs(Vector3.Angle(Cam_transf.forward, Vector3.up))< 65){
												...
												float diffRotY1 = currentRotY1 - Prev_cam_rot.y;
												particlesL[i].rotation = particlesL[i].rotation+diffRotY1;
												//Debug.Log("aa");
											}else{
												if(Mathf.Abs(Vector3.Angle(Cam_transf.forward, Vector3.up)) < 45){
													//particles[i].rotation = Mathf.Lerp(particles[i].rotation,particles[i].rotation+diffRot, Time.deltaTime*2);
												}else{
													particlesL[i].rotation = particlesL[i].rotation+diffRot1;
												}
											}
										}else{
											...
											if(diffRot == 0 && currentRotY!=Prev_cam_rot.y && Angle_check < 65){//Mathf.Abs(Vector3.Angle(Cam_transf.forward, Vector3.up))< 65){									
												...
												particlesL[i].rotation = particlesL[i].rotation-diffRotY*Dot_check;
												//Debug.Log("aa");
											}else{
												if(Angle_check < 45){
													//particles[i].rotation = Mathf.Lerp(particles[i].rotation,particles[i].rotation+diffRot, Time.deltaTime*2);
													particlesL[i].rotation = particlesL[i].rotation+diffRot-diffRotY*Dot_check;
												}else{
													particlesL[i].rotation = particlesL[i].rotation+diffRot-diffRotY*Dot_check;
												}
											}
							...
							DecoupledCounter ++;
								if(DecoupledCounter == particlesL.Length){
								DecoupledCounter = 0;
							}
