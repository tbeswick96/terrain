...
//				}
	...
	//			float _ColorDiff = oceanMat.GetFloat("_ColorDiff");
		...
		//			oceanMat.SetColor ("_ShadowColor", new Color(Mathf.Clamp01(FF.r),Mathf.Clamp01(FF.g),Mathf.Clamp01(FF.b),oceanMat.GetColor ("_ShadowColor").a));
					...
					if (WeatherDensity && Application.isPlaying) {
						if (SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.HeavyStorm) {
							oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, new Color (StormSunColor.r,StormSunColor.g,StormSunColor.b,EvalSunIntensity), 0.8f * Speed_factor * Time_delta + 0.1f));

						} else {
							oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, new Color (SkyManager.gradCloudLitColor.r, SkyManager.gradCloudLitColor.g, SkyManager.gradCloudLitColor.b, EvalSunIntensity), 0.8f * Speed_factor * Time_delta + 0.1f));
						}
					} else {
						oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, new Color (SkyManager.gradCloudLitColor.r, SkyManager.gradCloudLitColor.g, SkyManager.gradCloudLitColor.b, EvalSunIntensity), 0.8f * Speed_factor * Time_delta + 0.1f));
					}
	//				oceanMat.SetColor ("_SunColor", Color.Lerp(_SunColor, SkyManager.gradCloudLitColor, 0.8f*Speed_factor * Time_delta+0.1f));
				//	oceanMat.SetColor ("_ShadowColor", Color.Lerp(_ShadowColor, SkyManager.gradCloudShadeColor, 0.8f*Speed_factor * Time_delta+0.1f));
					oceanMat.SetColor ("_ShadowColor", Color.Lerp(_ShadowColor, new Color(SkyManager.gradCloudShadeColor.r,SkyManager.gradCloudShadeColor.g,SkyManager.gradCloudShadeColor.b,SkyManager.VolCloudTransp), 0.8f*Speed_factor * Time_delta+0.1f));
					//Debug.Log(SkyManager.calcColorTime + " "+IntensityDiff.Evaluate (SkyManager.calcColorTime));
					oceanMat.SetFloat ("_ColorDiff",EvalLightDiff);

					if (FogFromSkyGrad) {
						oceanMat.SetColor ("_FogColor", Color.Lerp (_FogColor, SkyManager.gradSkyColor, 4 * Speed_factor * Time_delta + 0.1f));
					} else {
						oceanMat.SetColor ("_FogColor", Color.Lerp (_FogColor, SkyManager.gradCloudFogColor, 4 * Speed_factor * Time_delta + 0.1f));
					}

					oceanMat.SetFloat ("_FogFactor", EvalFog);
					oceanMat.SetFloat ("_Coverage", Mathf.Lerp (_Coverage, Coverage, Speed_factor * Time_delta + 0.1f));
				}
				...
				if (WeatherDensity){// && Application.isPlaying) { //v3.4.3
					...
					if (SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.HeavyStorm) {
						...
						Coverage = Mathf.Lerp (Coverage, ClearDayCoverage, 0.8f * Speed_factor * Time_delta + 0.1f);
